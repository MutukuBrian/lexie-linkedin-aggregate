...
import { ExternalLink, Copy, ThumbsUp, MessageSquare, Share2, Globe, Clock } from 'lucide-react';
...
export const PostCard: React.FC<PostCardProps> = ({ post }) => {
  ...
  const avatar = parseJson<{ url?: string }>(post.avatar, {});
  const images: Array<{ url: string; width?: number; height?: number }> = parseJson(post.post_images, []);
  ...
  const copyForClaude = async () => {
    const text = `${settings.claudePrompt}\n\nPost:\n---\n${content}\n---\n\nPoster: ${post.poster_name}\nLink: ${post.post_url}`;
    try {
    ...
    >
      ...
      <div className="flex items-start gap-3 p-4 pb-3">
        ...
        <a href={post.poster_url ?? '#'} target="_blank" rel="noreferrer" className="flex-shrink-0">
          {avatar.url && !avatarErr ? (
            <img
              src={proxyImg(avatar.url, 48, 48)}
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <button
            ...
            className="btn-copy-gradient flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-[11px] font-semibold transition-all bg-zinc-100 text-zinc-600"
          >
      ...
      {post.website && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex items-center gap-3 px-4 py-3 bg-zinc-50 border-t border-zinc-100 hover:bg-zinc-100 transition-colors group"
        >
          <Globe className="w-4 h-4 text-zinc-400 flex-shrink-0" />
          <div className="min-w-0">
            <p className="text-xs font-semibold text-zinc-700 group-hover:text-[#0a66c2] truncate transition-colors">
              {post.website_label ?? post.website}
            </p>
            <p className="text-[10px] text-zinc-400 truncate">{post.website}</p>
          </div>
          <ExternalLink className="w-3 h-3 text-zinc-300 ml-auto flex-shrink-0" />
        </a>
      )}
